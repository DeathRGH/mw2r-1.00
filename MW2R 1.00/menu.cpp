...
void DrawMenuText() {
	...
	if (Options.menuPageIndex == submenus::Main) {
		...
		DrawToggle("debug_drawMatNames", &Options.debug_drawMatNames);
...
void DrawMenu() {
	...
	DrawText("Modern Warfare 2 Remastered 1.00", Options.menuX.current + 5, Options.menuY.current + 0x40, Options.menuFontSize.current * 1.2f, Options.color_menuText.color.ToFloatArray(), FontForIndex(1));
}
...
ScePadData padData;
void MonitorButtons() {
	uartprintf("[MW2R 1.00] MonitorButtons() -> THREAD STARTED!\n");
	while (ShouldRun()) {
		if (!userPadHandle) {
			Sleep(10);
			continue;
	...
	uartprintf("[MW2R 1.00] MonitorButtons() -> THREAD ENDED!\n");
}

void LoopSettings() {
	uartprintf("[MW2R 1.00] LoopSettings() -> THREAD STARTED!\n");
	while (ShouldRun()) {
		//Cache::CacheAll();
	...
	uartprintf("[MW2R 1.00] LoopSettings() -> THREAD ENDED!\n");
}
