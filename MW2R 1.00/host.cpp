...
void FireMagicBullet(short entNum, const char *projectile) {
	...
	playerAngles[0] = *(float *)((entNum * gentity_size) + gentity_t + 0x40);
	playerAngles[1] = *(float *)((entNum * gentity_size) + gentity_t + 0x40 + 4);
	playerAngles[2] = *(float *)((entNum * gentity_size) + gentity_t + 0x40 + 8);

	float forward[3];
	AngleToVectors(playerAngles, forward);

	float viewOrigin[3];
	viewOrigin[0] = *(float *)((entNum * gentity_size) + gentity_t + 0xF4);
	viewOrigin[1] = *(float *)((entNum * gentity_size) + gentity_t + 0xF4 + 4);
	viewOrigin[2] = *(float *)((entNum * gentity_size) + gentity_t + 0xF4 + 8) + 65.0f;
...
int ClosestClient(int i) {
	...
	for (int j = 0; j < MAX_CLIENTS; j++) {
		...
		if (Host::Entity::GetEntityPtr(j)->team == Host::Entity::GetEntityPtr(i)->team)
			continue;
...
void UnlockAllTrophies_Internal() {
	...
	for (int j = 0; j < 50; j++) {
		SV_GameSendServerCommand(trophyClientInternal, trophies[j]);
		Sleep(50);
		SV_GameSendServerCommand(trophyClientInternal, trophies[j]);
		Sleep(50);
		char temp[100];
		snprintf(temp, sizeof(temp), j == 49 ? "c \"Trophy [^2%i^7/^250^7] Unlocked\"" : "c \"Trophy [^3%i^7/^250^7] Unlocked\"", j + 1);
		SV_GameSendServerCommand(trophyClientInternal, temp);
		Sleep(1000);
	}
...
void UnlockAllTrophies(int i) {
	if (Menu::Menu[i].isUnlockingTrophies) {
		SV_GameSendServerCommand(i, "f \"^1Already unlocking trophies...\"");
		return;
	}
...
void PrintCoordinates(int i) {
	...
	SV_GameSendServerCommand(i, temp);
}
...
void SavePosition(int i) {
	...
	SV_GameSendServerCommand(i, "c \"Position: ^2Saved\"");
}

void LoadPosition(int i) {
	if (!Menu::Menu[i].savedPosition) {
		SV_GameSendServerCommand(i, "c \"^1Save a position first!\"");
		return;
	}
	...
	SV_GameSendServerCommand(i, "c \"Position: ^2Loaded\"");
}
...
gentity_s *FindCollision(const char *name) { //NOT UPDATED
	for (int i = 0; i < 2048; i++) {
		...
		if (*(uint8_t *)((uint8_t *)ent + 0x101) == 4) {
			...
			uartprintf("[MW2R 1.00] FindCollision(%s) -> %i %s\n", name, i, targetname);
			if (targetname) {
				// see d3dbsp files for maps
				if (!strcmp(targetname, name)) {
					uartprintf("[MW2R 1.00] FindCollision(%s) -> 0x%llX (%i): %s\n", name, ent, i, targetname);
					return ent;
				}
			}
		}
	...
	uartprintf("[MW2R 1.00] FindCollision(%s) -> Collision for \"%s\" not found!\n", name, name);
	return 0;
}
...
void Select(int i) {
	...
	case hostSub::SUB_SELF:
		...
		case 0:
			//Godmode
			SV_GameSendServerCommand(i, "c \"^1Not implemented!\"");
			break;
		case 1:
			Menu[i].infiniteAmmo = !Menu[i].infiniteAmmo;
			SV_GameSendServerCommand(i, Menu[i].infiniteAmmo ? "c \"Infinite Ammo [^2ON^7]\"" : "c \"Infinite Ammo [^1OFF^7]\"");
			break;
		case 2:
			//2x Movement Speed
			SV_GameSendServerCommand(i, "c \"^1Not implemented!\"");
	...
	case hostSub::SUB_LOBBY:
		...
		case 3:
			//AddFriendlyBot();
			SV_GameSendServerCommand(i, "c \"^2Friendly ^7Bot Added\"");
			break;
		case 4:
			//RemoveFriendlyBot();
			SV_GameSendServerCommand(i, "c \"^2Friendly ^7Bot Removed\"");
			break;
		case 5:
			//AddEnemyBot();
			SV_GameSendServerCommand(i, "c \"^1Enemy ^7Bot Added\"");
			break;
		case 6:
			//RemoveEnemyBot();
			SV_GameSendServerCommand(i, "c \"^1Enemy ^7Bot Removed\"");
			break;
		case 7:
			GScr_MapRestart();
			break;
	...
	case hostSub::SUB_AIMBOT:
		...
		case 0:
			...
			SV_GameSendServerCommand(i, Menu[i].unfairAimbot ? "c \"Unfair Aimbot [^2ON^7]\"" : "c \"Unfair Aimbot [^1OFF^7]\"");
			break;
		case 1:
	...
	case hostSub::SUB_PROJECTILE:
		...
		case 0:
			...
			SV_GameSendServerCommand(i, Menu[i].magicBullet ? "c \"Magic Bullet [^2ON^7]\"" : "c \"Magic Bullet [^1OFF^7]\"");
			break;
		case 1:
			Menu[i].magicBulletIndex = 0;
			SV_GameSendServerCommand(i, "c \"Bullet Type: ^2RPG\"");
			break;
		default:
			break;
		}
...
void PickupEntity(gentity_s *player) { //NOT UPDATED
	...
	int r = BulletTrace(start, end, &entityId);
	if (!r)
		clientCurrentEntity[0/*player->number*/] = Entity::GetEntityPtr(entityId);
}

void MoveForgeEntities() { //NOT UPDATED
	uartprintf("[MW2R 1.00] MoveForgeEntities() -> THREAD STARTED!\n");
	while (ShouldRun()) {
	...
	uartprintf("[MW2R 1.00] MoveForgeEntities() -> THREAD ENDED!\n");
}
